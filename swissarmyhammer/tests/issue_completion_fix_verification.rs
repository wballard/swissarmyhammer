...
async fn test_completion_detection_fix() {
    ...
    let _issue2 = issue_storage
        ...
        .await
        .create_issue(
            "active_issue_2".to_string(),
            "This is another active issue".to_string(),
        )
        .await
        .unwrap();
    ...
    fs::write(
        &nested_issue_file,
        "# Nested Issue\n\nThis should NOT be completed despite being in a path with 'complete'",
    )
    .unwrap();
    ...
    assert!(
        ...
        "Nested issue should NOT be completed despite 'complete' in path"
    );
