...
use crate::cost::{ApiCall, ApiCallId, CostError, CostSession, CostSessionId};
...
const STORAGE_CHANNEL_CLOSED_ERROR: &str = "Storage worker channel closed";
const STORAGE_OPERATION_CANCELLED_ERROR: &str = "Storage operation cancelled";
...
impl AsyncStorageManager {
    ...
    pub async fn store_session(&self, session: CostSession) -> Result<(), CostError> {
        ...
        if self.config.enable_write_behind {
            ...
            self.session_cache
                .put(session_key, session.clone(), true)
                .await;
            ...
            self.operation_tx
                ...
                .map_err(|_| CostError::SerializationError {
                    message: STORAGE_CHANNEL_CLOSED_ERROR.to_string(),
                })?;

            response_rx
                .await
                .map_err(|_| CostError::SerializationError {
                    message: STORAGE_OPERATION_CANCELLED_ERROR.to_string(),
                })?
        }
    }
    ...
    pub async fn flush(&self) -> Result<(), CostError> {
        self.operation_tx
            .send(StorageOperation::BatchFlush)
            .map_err(|_| CostError::SerializationError {
                message: STORAGE_CHANNEL_CLOSED_ERROR.to_string(),
            })?;
    ...
    async fn flush_batch(&mut self) {
        ...
        for operation in self.current_batch.operations.drain(..) {
            match operation {
                ...
                StorageOperation::UpdateApiCall {
                    session_id: _,
                    call_id: _,
                    api_call: _,
                    response_tx,
                ...
                StorageOperation::DeleteSession {
                    session_id: _,
                    response_tx,
                } => {
                    // Mock implementation
...
mod tests {
    ...
    async fn test_write_cache() {
        ...
        // Test put and get
        cache
            .put("key1".to_string(), "value1".to_string(), false)
            .await;
        ...
        // Test dirty entries
        cache
            .put("key2".to_string(), "value2".to_string(), true)
            .await;
