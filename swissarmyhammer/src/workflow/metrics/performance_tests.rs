...
mod performance_tests {
    ...
    fn test_cost_metrics_performance() {
        ...
        println!("Time for {} runs with cost tracking: {:?}", NUM_RUNS, elapsed_with_cost);

        // Verify all metrics were processed
        assert_eq!(metrics.run_metrics.len(), NUM_RUNS.min(super::super::cleanup::MAX_RUN_METRICS));
        ...
        assert!(elapsed_with_cost < Duration::from_secs(1), 
                "Performance test took too long: {:?}", elapsed_with_cost);
    }
    ...
    fn test_baseline_performance_without_cost() {
        ...
        println!("Time for {} runs without cost tracking: {:?}", NUM_RUNS, elapsed_without_cost);
        ...
        assert!(elapsed_without_cost < Duration::from_secs(1), 
                "Baseline performance test took too long: {:?}", elapsed_without_cost);
    }
    ...
    fn test_cost_aggregation_performance() {
        ...
        println!("Time for {} workflows with {} runs each: {:?}", 
                NUM_WORKFLOWS, RUNS_PER_WORKFLOW, elapsed);

        // Verify aggregation worked correctly
        assert!(metrics.workflow_metrics.len() > 0);
        let global_metrics = metrics.get_global_metrics();
        assert!(global_metrics.cost_metrics.is_some());
        
        // Check that cost trends were updated
        assert!(global_metrics.resource_trends.cost_trend.len() > 0);
        assert!(global_metrics.resource_trends.token_efficiency_trend.len() > 0);

        // Ensure aggregation performance is reasonable (should complete within 2 seconds)
        assert!(elapsed < Duration::from_secs(2), 
                "Cost aggregation performance test took too long: {:?}", elapsed);
    }
    ...
    fn test_cost_trend_memory_usage() {
        ...
        assert!(global_metrics.resource_trends.cost_trend.len() <= super::super::trends::MAX_TREND_DATA_POINTS);
        assert!(global_metrics.resource_trends.token_efficiency_trend.len() <= super::super::trends::MAX_TREND_DATA_POINTS);
        assert!(global_metrics.resource_trends.avg_cost_per_call_trend.len() <= super::super::trends::MAX_TREND_DATA_POINTS);
    }
}