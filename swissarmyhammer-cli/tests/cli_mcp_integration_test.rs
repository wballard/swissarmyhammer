...
async fn test_cli_can_call_mcp_tools() {
    ...
    assert!(context.has_tool("issue_create"), "issue_create tool should be available");
    assert!(context.has_tool("memo_create"), "memo_create tool should be available");
}
...
async fn test_issue_create_tool_integration() {
    ...
    let args = context.create_arguments(vec![
        ...
        ("content", json!("# Test Issue\n\nThis is a test issue for integration testing.")),
    ]);
    
    let result = context.execute_tool("issue_create", args).await;
    assert!(result.is_ok(), "Failed to execute issue_create tool: {:?}", result.err());
    
    let call_result = result.unwrap();
    assert_eq!(call_result.is_error, Some(false), "Tool execution reported an error");
    assert!(!call_result.content.is_empty(), "Tool result should have content");
}
...
async fn test_memo_create_tool_integration() {
    ...
    assert!(result.is_ok(), "Failed to execute memo_create tool: {:?}", result.err());
...
async fn test_nonexistent_tool_error() {
    ...
    assert!(error.to_string().contains("Tool not found"), "Error should mention tool not found");
}
...
async fn test_mcp_tool_runner_trait() {
    ...
    assert!(result.is_ok(), "McpToolRunner should execute successfully: {:?}", result.err());
...
async fn test_invalid_arguments_error() {
    ...
    let args = context.create_arguments(vec![
        ("invalid_field", json!("invalid_value")),
    ]);
...
async fn test_issue_workflow_integration() {
    ...
    match next_result {
        Ok(result) => {
            assert_eq!(result.is_error, Some(false), "issue_next should not report error when successful");
        }
        Err(e) => {
...
fn test_response_formatting_utilities() {
    ...
    let status_msg = response_formatting::create_status_message("test operation", true, Some("All good"));
    assert!(status_msg.contains("SUCCESS"));
    assert!(status_msg.contains("test operation"));
    assert!(status_msg.contains("All good"));
    
    let error_msg = response_formatting::create_status_message("failed operation", false, Some("Something failed"));
