                ...
                if should_have_cost && harness.calculator.supports_cost_calculation() {
                    ...
                    if input_tokens > 0 || output_tokens > 0 {
                        assert!(calculation.total_cost > Decimal::ZERO,
                            ...
                            input_tokens, output_tokens, calculation.total_cost);
                    }
                }
            }
            Err(_) => {
...
async fn property_token_count_consistency() {
    ...
    let test_patterns = vec![
        (0, 0, r#"{"usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}}"#),
        (1, 1, r#"{"usage": {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2}}"#),
        (10, 5, r#"{"usage": {"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15}}"#),
        (100, 50, r#"{"usage": {"prompt_tokens": 100, "completion_tokens": 50, "total_tokens": 150}}"#),
        (1000, 2000, r#"{"usage": {"prompt_tokens": 1000, "completion_tokens": 2000, "total_tokens": 3000}}"#),
    ];

    for (expected_input, expected_output, api_response) in test_patterns {
        // Create expected usage for comparison
        let expected_usage = TokenUsage::from_estimation(expected_input, expected_output, ConfidenceLevel::Medium);
        
        // Test token counting from API response
        let result = token_counter.count_from_response(api_response, Some(expected_usage), "test-model");
        ...
        assert_eq!(actual_usage.input_tokens + actual_usage.output_tokens, actual_usage.total_tokens,
            "Token sum should be consistent: {} + {} = {}", 
            actual_usage.input_tokens, actual_usage.output_tokens, actual_usage.total_tokens);

        // Property: Token values should be non-negative
        assert!(actual_usage.input_tokens >= 0, "Input tokens should be non-negative");
        assert!(actual_usage.output_tokens >= 0, "Output tokens should be non-negative");
        assert!(actual_usage.total_tokens >= 0, "Total tokens should be non-negative");
    }
}
...
async fn property_session_state_transitions() {
    ...
    for (scenario_id, final_states) in test_scenarios.iter().enumerate() {
        for final_state in final_states {
            let issue_id = IssueId::new(format!("property-state-{}-{:?}", scenario_id, final_state)).unwrap();
            ...
            assert!(session_id.is_ok(), "Session creation should succeed with valid issue ID");
            ...
            assert_eq!(session.status, CostSessionStatus::Active, "New session should be active");
            ...
            assert!(add_result.is_ok(), "Should be able to add API calls to active session");
            ...
            assert!(final_session.is_some(), "Completed session should still exist");
            let final_session = final_session.unwrap();
            assert_eq!(final_session.status, final_state.clone(), "Final session state should match requested state");
            ...
            assert!(modify_result.is_err(), "Should not be able to modify completed session");
        }
    }
}
...
async fn property_data_consistency_across_operations() {
    ...
    for i in 0..session_count {
        ...
        let api_calls = harness.api_call_generator.generate_multiple_calls(num_api_calls as u32);
        ...
        session_data.insert(session_id, (api_calls.len(), expected_input_tokens, expected_output_tokens));
    }
    ...
    for (session_id, (expected_calls, expected_input, expected_output)) in &session_data {
        ...
        assert!(session.is_some(), "Session should exist for ID: {:?}", session_id);
        let session = session.unwrap();

        // Property: API call count should match what was added
        assert_eq!(session.api_calls.len(), *expected_calls, 
            "API call count should match for session {:?}: expected {}, got {}", 
            session_id, expected_calls, session.api_calls.len());

        // Property: Token counts should sum correctly
        let actual_input: u32 = session.api_calls.iter()
            .map(|(_, call)| call.input_tokens)
            .sum();
        let actual_output: u32 = session.api_calls.iter()
        ...
        for (call_idx, (_, api_call)) in session.api_calls.iter().enumerate() {
            assert!(api_call.started_at <= api_call.completed_at.unwrap_or(api_call.started_at),
                "API call {} in session {:?} should have valid timestamps", call_idx, session_id);
        }
    }
}
...
async fn property_session_cleanup_completeness() {
    ...
    assert_eq!(tracker_guard.session_count(), session_count, 
        "Total session count should equal created sessions");

    // Property: Completed count should match what we completed
    assert_eq!(tracker_guard.completed_session_count(), completed_sessions.len(), 
    ...
    assert_eq!(tracker_guard.active_session_count(), expected_active, 
    ...
    for session_id in &completed_sessions {
        ...
        assert!(session.is_some(), "Completed session {:?} should be accessible", session_id);
        assert_eq!(session.unwrap().status, CostSessionStatus::Completed, 
...
async fn property_pricing_model_consistency() {
    ...
    for (pricing_model, should_calculate_cost) in test_cases {
        ...
        let mut session = crate::cost::CostSession::new(
            IssueId::new("property-pricing".to_string()).unwrap(),
        );
        ...
        assert_eq!(calculator.supports_cost_calculation(), should_calculate_cost,
...
async fn property_concurrent_data_integrity() {
    ...
    let handles: Vec<_> = (0..num_workers).map(|worker_id| {
        ...
        tokio::spawn(async move {
            ...
            for op_id in 0..operations_per_worker {
                let issue_id = IssueId::new(format!("property-concurrent-{}-{}", worker_id, op_id)).unwrap();
                let session_id = {
    ...
    assert_eq!(tracker_guard.session_count(), all_sessions.len(),
    ...
    for (session_id, expected_tokens) in &all_sessions {
        let session = tracker_guard.get_session(session_id);
        assert!(session.is_some(), "Concurrent session {:?} should exist", session_id);
        
        let session = session.unwrap();
        assert_eq!(session.api_calls.len(), 3, "Each session should have 3 API calls");
        
        let actual_tokens: u32 = session.api_calls.iter()
...
async fn property_api_call_data_preservation() {
    ...
    let test_configurations = vec![
        ("https://short.url", "model1", 100, 50, ApiCallStatus::Success, None),
        ("https://very-long-url.example.com/api/v1/messages/with/many/path/segments", "claude-3-sonnet-20241022", 0, 0, ApiCallStatus::Failed, Some("Error message".to_string())),
        ("https://api.example.com", "unknown-model-name", 1000000, 500000, ApiCallStatus::Success, None),
        ("https://api.example.com", "", 1, 1, ApiCallStatus::Timeout, Some("Request timeout".to_string())),
    ];
    ...
    for (url, model, input_tokens, output_tokens, status, error_msg) in test_configurations {
        ...
        if let Ok(mut api_call) = api_call_result {
            api_call.complete(input_tokens, output_tokens, status.clone(), error_msg.clone());
            ...
            if add_result.is_ok() {
                ...
                let session = session.unwrap();
                assert!(!session.api_calls.is_empty(), "API call should be preserved");
                
                // Get any API call from the session to verify data preservation
                let (_, preserved_call) = session.api_calls.iter().next().expect("Session should have at least one API call");
                ...
                assert_eq!(preserved_call.input_tokens, input_tokens, "Input tokens should be preserved");
                assert_eq!(preserved_call.output_tokens, output_tokens, "Output tokens should be preserved");
                assert_eq!(preserved_call.status, status, "Status should be preserved");
                
                if let Some(ref expected_error) = error_msg {
                    assert_eq!(preserved_call.error_message.as_ref(), Some(expected_error), "Error message should be preserved");
                } else {
                    assert!(preserved_call.error_message.is_none(), "No error message should be preserved when none provided");
                }

                // Property: Timestamps should be valid
                assert!(preserved_call.started_at <= preserved_call.completed_at.unwrap_or(preserved_call.started_at),
