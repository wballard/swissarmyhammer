...
mod tests {
    use crate::cost::{
        ApiCall, ApiCallStatus, ConfidenceLevel, CostCalculator, CostTracker, IssueId,
    ...
    fn test_end_to_end_token_counting_workflow() {
        ...
        let api_usage = token_counter
            .count_from_response(response_json, None, "claude-3-sonnet-20241022")
            .unwrap();
        ...
        cost_tracker.add_api_call(&session_id, api_call).unwrap();
    ...
    fn test_token_estimation_fallback() {
        ...
        let extraction_result = token_counter
            .api_extractor
            .extract_from_response(response_without_usage);
    ...
    fn test_token_validation_workflow() {
        ...
        let input_text =
            "This is a longer input text that should be estimated to verify API accuracy.";
    ...
    fn test_multiple_api_calls_with_different_token_sources() {
        ...
        let mut api_call1 =
            ApiCall::new("https://api.anthropic.com/v1/messages", "claude-3-sonnet").unwrap();

        let response1 = r#"{"usage": {"input_tokens": 100, "output_tokens": 75}}"#;
        let usage1 = token_counter
            .count_from_response(response1, None, "claude-3-sonnet")
            .unwrap();
        ...
        let mut api_call2 =
            ApiCall::new("https://api.anthropic.com/v1/messages", "claude-3-haiku").unwrap();
    ...
    fn test_token_counting_performance() {
        ...
        let large_text =
            "This is a much longer text that repeats content to test performance characteristics. "
                .repeat(100);
        ...
        assert!(
            duration.as_secs() < 1,
            "Token estimation took too long: {:?}",
            duration
        );
    ...
    fn test_comprehensive_validation_accuracy() {
        ...
        for (input_text, output_text, api_input, api_output) in test_cases {
            ...
            // Perform validation
            let _api_usage = token_counter
                .count_from_response(&api_response, Some(estimated), "claude-3-sonnet")
                .unwrap();
        }
