...
mod tests {
    ...
    fn test_action_parsing_with_liquid_templates() {
        ...
        let description = r#"Execute prompt "say-hello" with name="{{ name }}" language="{{ language }}""#;
        let action = parse_action_from_description_with_context(description, &context)
            .unwrap()
            .unwrap();
        
        let prompt_action = action.as_any().downcast_ref::<crate::workflow::PromptAction>().unwrap();
    ...
    fn test_action_parsing_with_default_values() {
        ...
        let log_action = action.as_any().downcast_ref::<crate::workflow::LogAction>().unwrap();
    ...
    fn test_action_parsing_with_undefined_filter() {
        ...
        assert_eq!(log_action.message, "Value: {{ value | nonexistent_filter }}");
    }
    ...
    fn test_prompt_action_with_template_in_arguments() {
        ...
        let description = r#"Execute prompt "assistant" with message="Help {{ user }} to {{ task }}""#;
        let action = parse_action_from_description_with_context(description, &context)
            .unwrap()
            .unwrap();
        ...
        assert_eq!(prompt_action.arguments.get("message").unwrap(), "Help Alice to review code");
    }
    ...
    fn test_action_parsing_with_complex_object_template_value() {
        let mut context = HashMap::new();
        let mut template_vars = HashMap::new();
        template_vars.insert("user".to_string(), json!({
            "name": "Bob",
            "id": 123
        }));
    ...
    fn test_action_parsing_with_special_characters_in_template() {
        ...
        assert_eq!(log_action.message, "Message: Hello World & <everyone>! at /usr/bin/test");
    }
    ...
    fn test_set_variable_action_with_template() {
        ...
        let set_action = action.as_any().downcast_ref::<crate::workflow::SetVariableAction>().unwrap();
