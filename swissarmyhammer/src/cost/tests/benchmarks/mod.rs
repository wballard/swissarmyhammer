...
impl PerformanceTestResult {
    ...
    pub fn meets_performance_criteria(&self, benchmarks: &PerformanceBenchmarks) -> bool {
        ...
        self.peak_memory_usage <= benchmarks.max_memory_per_session * self.operation_count as usize &&
    ...
    for i in 0..operation_count {
            ...
            if i > 0 && i % 100 == 0 {
                ...
                if current_ops_per_sec < benchmarks.min_ops_per_second * 0.8 {
                    warn!("Performance degrading: {:.1} ops/sec at iteration {}", current_ops_per_sec, i);
                }
            }
        }

        let total_duration = start_time.elapsed();
        let ops_per_second = PerformanceTestResult::operations_per_second(operation_count, total_duration);
        ...
        percentiles.insert("session_creation_p50".to_string(), creation_times_sorted[creation_times_sorted.len() / 2]);
        percentiles.insert("session_creation_p95".to_string(), creation_times_sorted[creation_times_sorted.len() * 95 / 100]);
        percentiles.insert("session_creation_p99".to_string(), creation_times_sorted[creation_times_sorted.len() * 99 / 100]);

        // API call addition percentiles
        let mut api_call_times_sorted = api_call_add_times.clone();
        api_call_times_sorted.sort();
        percentiles.insert("api_call_add_p50".to_string(), api_call_times_sorted[api_call_times_sorted.len() / 2]);
        percentiles.insert("api_call_add_p95".to_string(), api_call_times_sorted[api_call_times_sorted.len() * 95 / 100]);

        // Cost calculation percentiles
        let mut calc_times_sorted = cost_calculation_times.clone();
        calc_times_sorted.sort();
        percentiles.insert("cost_calc_p50".to_string(), calc_times_sorted[calc_times_sorted.len() / 2]);
        percentiles.insert("cost_calc_p95".to_string(), calc_times_sorted[calc_times_sorted.len() * 95 / 100]);
        ...
        assert!(creation_times_sorted[creation_times_sorted.len() * 95 / 100] <= benchmarks.max_session_creation_time,
            "95th percentile session creation time {:?} exceeds benchmark {:?}",
            creation_times_sorted[creation_times_sorted.len() * 95 / 100], benchmarks.max_session_creation_time);

        assert!(api_call_times_sorted[api_call_times_sorted.len() * 95 / 100] <= benchmarks.max_api_call_add_time,
            "95th percentile API call addition time {:?} exceeds benchmark {:?}",
            api_call_times_sorted[api_call_times_sorted.len() * 95 / 100], benchmarks.max_api_call_add_time);

        assert!(calc_times_sorted[calc_times_sorted.len() * 95 / 100] <= benchmarks.max_cost_calculation_time,
            "95th percentile cost calculation time {:?} exceeds benchmark {:?}",
            calc_times_sorted[calc_times_sorted.len() * 95 / 100], benchmarks.max_cost_calculation_time);

        assert!(ops_per_second >= benchmarks.min_ops_per_second,
            "Operations per second {:.1} below benchmark {:.1}",
            ops_per_second, benchmarks.min_ops_per_second);

        info!("Session lifecycle benchmark: {:.1} ops/sec, avg time: {:?}", ops_per_second, result.average_operation_time);
...
async fn benchmark_concurrent_performance() {
    ...
    for concurrency in concurrency_levels {
        ...
        let worker_handles: Vec<_> = (0..concurrency).map(|worker_id| {
            ...
            tokio::spawn(async move {
                ...
                for op_id in 0..operations_per_worker {
                    let issue_id = IssueId::new(format!("concurrent-bench-{}-{}", worker_id, op_id)).unwrap();
                ...
                let worker_ops_per_sec = worker_operations as f64 / worker_duration.as_secs_f64();

                Ok::<_, Box<dyn std::error::Error + Send + Sync>>((worker_operations, worker_duration, worker_ops_per_sec))
            })
        }).collect();
        ...
        performance_results.insert(concurrency, PerformanceTestResult {
        ...
        info!("Concurrency level {}: {:.1} ops/sec, {:.1}% success rate", 
            concurrency, overall_ops_per_sec, (successful_workers as f64 / concurrency as f64) * 100.0);
        ...
        if concurrency > 1 {
            ...
            if total_operations > 0 {
                assert!(efficiency >= min_efficiency,
                    ...
                    efficiency, concurrency, min_efficiency);
            }
        }
    }
...
async fn benchmark_scalability_limits() {
    ...
    let test_future = async {
        ...
        for session_count in session_counts {
            ...
            for i in 0..session_count {
                ...
                if i > 0 && i % 100 == 0 {
                    ...
                    if current_rate < benchmarks.min_ops_per_second * 0.5 {
                        warn!("Scalability degrading at {} sessions: {:.1} ops/sec", i, current_rate);
                    }
            ...
            scalability_results.insert(session_count, PerformanceTestResult {
            ...
            info!("Scalability test with {} sessions: {:.1} ops/sec, {:.1}% success rate", 
                session_count, ops_per_second, success_rate * 100.0);
            ...
            if successful_sessions > 0 {
                assert!(success_rate >= expected_min_rate, 
                    "Success rate {:.2} should meet minimum expectation {:.2} for {} sessions", 
                    success_rate, expected_min_rate, session_count);
        ...
        let performance_retention = large_scale.operations_per_second / baseline.operations_per_second;
        ...
        assert!(performance_retention >= min_retention, 
            "Should retain at least {:.1}% of baseline performance at scale: {:.2}",
            min_retention * 100.0, performance_retention);

        info!("Scalability analysis: {:.1}% performance retention at 10x scale", 
    ...
    assert!(result.is_ok(), "Scalability benchmark should succeed: {:?}", result);
}
...
async fn benchmark_memory_usage_patterns() {
    ...
    let test_future = async {
        ...
        for (pattern_name, session_count, calls_per_session) in test_patterns {
            ...
            info!("Starting pattern '{}': initial_sessions={}, expected to create={}", 
            ...
            memory_results.insert(pattern_name.to_string(), (
                creation_time,
                cleanup_time, 
                peak_sessions,
                final_sessions,
            ...
            info!("Memory pattern '{}': peak {} sessions, final {} sessions, active {} sessions", 
                pattern_name, peak_sessions, final_sessions, active_sessions);

            // Validate memory management
            assert_eq!(final_sessions, peak_sessions, "All sessions should be retained");
            ...
            assert!(
                ...
                active_sessions, expected_active
            );
        }
    ...
    assert!(result.is_ok(), "Memory usage benchmark should succeed: {:?}", result);
}
...
async fn benchmark_cost_calculation_performance() {
    ...
    for (scenario_name, session_count, calls_per_session) in test_scenarios {
        ...
        for i in 0..session_count {
            ...
            assert!(calculation.input_cost >= Decimal::ZERO, "Input cost should be non-negative");
            assert!(calculation.output_cost >= Decimal::ZERO, "Output cost should be non-negative");
        }
        ...
        info!("Cost calculation benchmark '{}': {:.1} calc/sec, P95: {:?}, P99: {:?}", 
            scenario_name, calculations_per_sec, p95, p99);

        // Validate performance benchmarks
        assert!(p95 <= benchmarks.max_cost_calculation_time,
            "95th percentile calculation time {:?} exceeds benchmark {:?} for scenario '{}'",
            p95, benchmarks.max_cost_calculation_time, scenario_name);

        assert!(calculations_per_sec >= benchmarks.min_ops_per_second,
            "Calculation rate {:.1}/sec below benchmark {:.1}/sec for scenario '{}'",
            calculations_per_sec, benchmarks.min_ops_per_second, scenario_name);

        // Verify that calculation time doesn't increase significantly with session size
        let avg_time_per_api_call = calculation_times.iter().sum::<Duration>() / calculation_times.len() as u32 / calls_per_session as u32;
        
        assert!(avg_time_per_api_call <= Duration::from_micros(500),
            "Average calculation time per API call {:?} too high for scenario '{}'",
            avg_time_per_api_call, scenario_name);
    }
}
...
pub fn detect_performance_regression(
    ...
    let performance_ratio = current_result.operations_per_second / baseline_result.operations_per_second;
...
async fn benchmark_comprehensive_system_performance() {
    ...
    let test_future = async {
        ...
        while start_time.elapsed() < workload_duration {
            ...
            match operation_type {
                ...
                1 => {
                    ...
                    let issue_id = IssueId::new(format!("medium-{}", total_operations)).unwrap();
                    let session_id = {
                        let mut tracker = shared_tracker.lock().await;
                        match tracker.start_session(issue_id) {
                            Ok(id) => id,
        ...
        info!("Comprehensive performance: {:.1} ops/sec, {:.1}% success rate over {} operations",
            ops_per_second, success_rate * 100.0, total_operations);

        Ok(comprehensive_result)
    };

    let result: Result<_, Box<dyn std::error::Error + Send + Sync>> = test_future.await;
    assert!(result.is_ok(), "Comprehensive performance test should succeed: {:?}", result);
}