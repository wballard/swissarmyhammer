...
impl FileSystem for StdFileSystem {
    ...
    fn write(&self, path: &Path, content: &str) -> Result<()> {
        ...
        std::fs::rename(&temp_path, path).map_err(|e| {
            ...
            SwissArmyHammerError::Io(std::io::Error::new(
                e.kind(),
                format!("Failed to rename temp file '{}' to '{}': {}", temp_path.display(), path.display(), e),
            ))
        })
    }
    ...
    fn read_dir(&self, path: &Path) -> Result<Vec<PathBuf>> {
        ...
        for entry in entries {
            let entry = entry.map_err(|e| {
                SwissArmyHammerError::Io(std::io::Error::new(
                    e.kind(),
                    format!("Failed to read directory entry in '{}': {}", path.display(), e),
                ))
            })?;
...
impl FileSystemUtils {
        ...
        T: serde::de::DeserializeOwned,
    {
        ...
        serde_yaml::from_str(&content).map_err(|e| {
            SwissArmyHammerError::Serialization(e)
        })
    }
    ...
    impl FileSystem for MockFileSystem {
        fn read_to_string(&self, path: &Path) -> Result<String> {
            self.files.lock().unwrap()
                .get(path)
                .cloned()
                .ok_or_else(|| SwissArmyHammerError::Io(
                    std::io::Error::new(
