...
impl TestEnvironment {
    async fn new() -> Self {
        ...
        let issue_storage = Box::new(FileSystemIssueStorage::new(issues_dir).unwrap());
        let issue_storage = Arc::new(RwLock::new(issue_storage as Box<dyn IssueStorage>));
        
        // Initialize git operations
        let git_ops = Arc::new(tokio::sync::Mutex::new(Some(GitOperations::with_work_dir(
...
async fn test_complete_issue_workflow() {
    ...
    let issue = env.issue_storage.write().await
        .create_issue(
    ...
    assert!(updated_issue.content.contains("JWT authentication implementation completed"));
    
    // Step 5: Mark issue as complete
    let completed_issue = env.issue_storage.write().await
...
async fn test_error_handling_scenarios() {
    ...
    // Test creating issue with empty name (should work but sanitize to "unnamed")
    let result = env.issue_storage.write().await
    ...
    // Test creating issue with dangerous characters in name
    let result = env.issue_storage.write().await
        .create_issue("../../../etc/passwd".to_string(), "Valid content".to_string())
        .await;
    assert!(result.is_ok());
    let issue = result.unwrap();
    assert_eq!(issue.name, "../../../etc/passwd");
    
    // Test working on non-existent issue
    let result = env.issue_storage.read().await
        .get_issue(999)
        .await;
...
async fn test_git_integration_edge_cases() {
    ...
    let issue2 = env.issue_storage.write().await
        .create_issue(
    ...
    if let Some(git) = git_ops.as_ref() {
        ...
        println!("Create work branch result: {:?}", result);
    }
    drop(git_ops);
...
async fn test_performance_with_many_issues() {
    ...
    // Create 50 issues
    for i in 1..=50 {
        let _ = env.issue_storage.write().await
            .create_issue(
...
async fn test_issue_file_structure() {
    ...
    let issue_file = env.temp_dir.path().join("issues").join(format!("{:06}_{}.md", issue.number, issue.name));
    ...
    let complete_file = env.temp_dir.path().join("issues").join("complete").join(format!("{:06}_{}.md", issue.number, issue.name));
