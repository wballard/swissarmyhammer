...
impl FailureInjector {
    ...
    pub async fn maybe_inject_failure(&self, operation_id: u32) -> Result<(), ChaosError> {
        ...
        if self.network_failures.load(Ordering::Relaxed) && operation_id % 3 == 0 {
            ...
            return Err(ChaosError::NetworkFailure("Simulated network partition".to_string()));
        }

        // Inject storage failures
        if self.storage_failures.load(Ordering::Relaxed) && operation_id % 5 == 0 {
            self.failure_count.fetch_add(1, Ordering::Relaxed);
            return Err(ChaosError::StorageFailure("Simulated storage backend failure".to_string()));
        }

        // Inject memory pressure
        if self.memory_pressure.load(Ordering::Relaxed) && operation_id % 7 == 0 {
            self.failure_count.fetch_add(1, Ordering::Relaxed);
            return Err(ChaosError::ResourceExhaustion("Simulated memory exhaustion".to_string()));
        }
...
async fn test_random_api_call_failures() {
    ...
    let test_future = async {
        ...
        let injector = failure_injector.clone();
                        ...
                        match session_result {
                            Ok(session_id) => {
                                ...
                                for call_id in 0..3 {
                                    ...
                                    if let Err(_) = injector.maybe_inject_failure(operation_id + call_id).await {
                                        call_failures += 1;
                                        continue;
                                    }

                                    let mut api_call = ApiCall::new(
                                        format!("https://api.anthropic.com/chaos/{}/{}", operation_id, call_id),
                                    ...
                                    match api_call {
                                        Ok(mut api_call) => {
                                            ...
                                            let (input_tokens, output_tokens) = if status == ApiCallStatus::Success {
                                                (100 + call_id * 50, 50 + call_id * 25)
                                            } else {
                                                (0, 0) // Failed calls have no tokens
                                            };

                                            api_call.complete(input_tokens, output_tokens, status, None);
            ...
            info!("Chaos test results - Sessions: {} successful, {} failed", successful_sessions, failed_sessions);
            info!("Injector stats: {} successes, {} failures, {:.2}% success rate", 
                injector_successes, injector_failures, injector_success_rate * 100.0);
            ...
            assert!(session_success_rate >= config.min_success_rate, 
                "Session success rate {:.2}% should be above minimum {:.2}%", 
                session_success_rate * 100.0, config.min_success_rate * 100.0);
    ...
    match &result {
        Ok(Ok(_)) => tracing::info!("Test scenario 'random_api_failures' completed successfully in {:?}", duration),
        Ok(Err(e)) => tracing::error!("Test scenario 'random_api_failures' failed after {:?}: {}", duration, e),
        Err(_) => tracing::error!("Test scenario 'random_api_failures' timed out after {:?}", duration),
    }

    assert!(result.is_ok(), "Random failure chaos test should complete: {:?}", result);
    let scenario_result: Result<(i32, i32, f64), Box<dyn std::error::Error + Send + Sync>> = result.unwrap();
    assert!(scenario_result.is_ok(), "Random failures should be handled gracefully: {:?}", scenario_result);
}
...
async fn test_storage_backend_interruptions() {
    ...
    let test_future = async {
            ...
            match session_result {
                Ok(session_id) => {
                    ...
                    for (call_idx, api_call) in api_calls.into_iter().enumerate() {
                        ...
                        if let Err(_) = injector.maybe_inject_failure(i * 10 + call_idx as u32).await {
                            continue;
                        }
        ...
        assert!(success_rate > 0.0, "Some operations should succeed despite storage failures");
        ...
        assert!(session_count >= successful_operations, 
            "System should maintain session tracking despite storage failures");

        info!("Storage chaos results: {} successful, {} failed, {:.1}% success rate", 
            successful_operations, failed_operations, success_rate * 100.0);
    ...
    match &result {
        Ok(_) => tracing::info!("Test scenario 'storage_interruptions' completed successfully in {:?}", duration),
        Err(e) => tracing::error!("Test scenario 'storage_interruptions' failed after {:?}: {}", duration, e),
    }

    assert!(result.is_ok(), "Storage interruption test should complete: {:?}", result);
}
...
async fn test_concurrent_failures_under_load() {
    ...
    let result = timeout(Duration::from_secs(90), async {
        ...
        let worker_handles: Vec<_> = (0..num_workers).map(|worker_id| {
            ...
            tokio::spawn(async move {
                ...
                for op_id in 0..20 {
                    ...
                    if let Err(chaos_error) = injector_clone.maybe_inject_failure(global_op_id).await {
                        match chaos_error {
                            ChaosError::NetworkFailure(_) => {
                                worker_failures += 1;
                    ...
                    let issue_id = IssueId::new(format!("concurrent-chaos-{}-{}", worker_id, op_id));
                        ...
                        Ok(issue_id) => {
                            ...
                            match session_result {
                                Ok(session_id) => {
                                    ...
                                    let mut api_call = ApiCall::new(
                                        format!("https://api.anthropic.com/concurrent/{}/{}", worker_id, op_id),
                                        "claude-3-sonnet-20241022",
                                    )?;
                ...
                Ok::<_, Box<dyn std::error::Error + Send + Sync>>((worker_successes, worker_failures))
            })
        }).collect();
        ...
        assert!(success_rate >= config.min_success_rate * 0.5, // Reduced expectation under extreme load
        ...
        let final_tracker = shared_tracker.lock().await;
        let session_count = final_tracker.session_count();
        assert!(session_count > 0, "System should have processed some sessions despite chaos");

        let (injector_successes, injector_failures, injector_rate) = failure_injector.get_stats();
        info!("Concurrent chaos test: {} successes, {} failures ({:.1}% success rate)", 
            total_successes, total_failures, success_rate * 100.0);
        info!("Injector contributed: {} successes, {} failures ({:.1}% success rate)",
            injector_successes, injector_failures, injector_rate * 100.0);

        Ok::<(i32, i32, f64), Box<dyn std::error::Error + Send + Sync>>((total_successes, total_failures, success_rate))
    }).await;

    assert!(result.is_ok(), "Concurrent failure test should complete: {:?}", result);
    let (successes, failures, rate) = result.unwrap().unwrap();
    assert!(successes > 0, "Should have some successful operations under chaos");
}
...
async fn test_recovery_after_failures() {
    ...
    let test_future = async {
        ...
        while failure_start.elapsed() < Duration::from_secs(15) {
            if let Err(_) = failure_injector.maybe_inject_failure(operations_during_failure).await {
        ...
        info!("Recovery results: {} successes, {} failures in {:?} ({:.1}% success rate)", 
            recovery_successes, recovery_failures, recovery_time, recovery_success_rate * 100.0);

        // Recovery should be relatively quick and effective
        assert!(recovery_time <= config.max_recovery_time, 
        ...
        assert!(total_sessions >= recovery_successes, 
            "System should track all successful recovery sessions");

        Ok((operations_during_failure, recovery_successes, recovery_time, recovery_success_rate))
    };
    ...
    match &result {
        Ok(_) => tracing::info!("Test scenario 'failure_recovery' completed successfully in {:?}", duration),
        Err(e) => tracing::error!("Test scenario 'failure_recovery' failed after {:?}: {}", duration, e),
    }

    assert!(result.is_ok(), "Failure recovery test should complete: {:?}", result);
}
...
async fn test_resource_limit_violations() {
    ...
    let test_future = async {
        ...
        for i in 0..excessive_sessions {
            ...
            if let Err(ChaosError::ResourceExhaustion(_)) = failure_injector.maybe_inject_failure(i).await {
        ...
        assert!(successful_creations > 0, "Should create some sessions even under resource pressure");
        assert!(resource_violations > 0, "Should encounter some resource violations as expected");

        let violation_rate = resource_violations as f64 / excessive_sessions as f64;
        assert!(violation_rate < 0.9, "Violation rate {:.1}% should not be excessive", violation_rate * 100.0);

        // System should remain responsive
        let final_tracker = shared_tracker.lock().await;
        let session_count = final_tracker.session_count();
        assert!(session_count >= successful_creations / 2, 
            "System should maintain reasonable number of sessions despite resource pressure");

        info!("Resource limit test: {} successful creations, {} violations ({:.1}% violation rate)",
            successful_creations, resource_violations, violation_rate * 100.0);
    ...
    match &result {
        Ok(_) => tracing::info!("Test scenario 'resource_limits' completed successfully in {:?}", duration),
        Err(e) => tracing::error!("Test scenario 'resource_limits' failed after {:?}: {}", duration, e),
    }

    assert!(result.is_ok(), "Resource limit test should complete: {:?}", result);
}