...
use swissarmyhammer::cost::performance::{
    ...
    optimization::{PerformanceOptimizer, PerformanceConfigBuilder},
};
use swissarmyhammer::cost::{IssueId, ApiCallStatus};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Benchmark Validation: {}", 
        if benchmarks_pass { "✅ PASS" } else { "❌ FAIL" });
    ...
    for i in 0..100 {
        ...
        if api_duration.as_millis() > API_OVERHEAD_TARGET_MS as u128 {
            println!("⚠️  API call {} exceeded target: {}ms > {}ms", 
                i, api_duration.as_millis(), API_OVERHEAD_TARGET_MS);
        }
    }
    
    optimizer.complete_session(&session_id, swissarmyhammer::cost::CostSessionStatus::Completed)?;
    ...
    println!("Performance Optimizer Validation: {}", 
        if optimizer_pass { "✅ PASS" } else { "❌ FAIL" });
    ...
    println!("  • Memory Usage: {:.2}%", validation_result.memory_usage_pct);
    println!("  • Cache Hit Rate: {:.1}%", validation_result.cache_hit_rate_pct);
    
    let validation_pass = validation_result.targets_met && 
        validation_result.memory_usage_pct <= MEMORY_LIMIT_PCT;
    ...
    println!("\n🎯 FINAL RESULT: {}", 
        if overall_pass { "✅ ALL TARGETS MET" } else { "❌ PERFORMANCE TARGETS NOT MET" });
