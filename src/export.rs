...
pub async fn run_export_command(
    ...
    match format {
        ExportFormat::TarGz => {
            exporter.export_tar_gz(&prompts, &output_path, metadata).await?;
        }
        ExportFormat::Zip => {
            exporter.export_zip(&prompts, &output_path, metadata).await?;
        }
        ExportFormat::Directory => {
            exporter.export_directory(&prompts, &output_path, metadata).await?;
        }
    }
    
    println!("Successfully exported {} prompts to {}", prompts.len(), output_path.display());
...
impl Exporter {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self { storage, _loader: loader })
    }
        ...
        for (name, prompt) in self.storage.iter() {
            ...
            if let Some(ref filter_source) = source_filter {
                let prompt_source_matches = match filter_source {
                    PromptSource::Builtin => matches!(prompt.source, crate::prompts::PromptSource::BuiltIn),
                    PromptSource::User => matches!(prompt.source, crate::prompts::PromptSource::User),
                    PromptSource::Local => matches!(prompt.source, crate::prompts::PromptSource::Local),
                };
    ...
    async fn create_manifest(&self, prompts: &[(String, PathBuf)]) -> Result<ExportManifest> {
        ...
        for (name, path) in prompts {
            ...
            if let Some(prompt) = self.storage.get(name) {
                prompt_metadata.push(PromptMetadata {
                    name: name.clone(),
                    title: prompt.title.clone(),
                    description: prompt.description.clone(),
                    source: match prompt.source {
                        crate::prompts::PromptSource::BuiltIn => "builtin".to_string(),
                        crate::prompts::PromptSource::User => "user".to_string(),
                        crate::prompts::PromptSource::Local => "local".to_string(),
                    },
...
mod tests {
    ...
    fn test_determine_output_path() {
        let result = determine_output_path(&Some("custom.tar.gz".to_string()), &ExportFormat::TarGz);
        assert!(result.is_ok());
    ...
    async fn test_export_directory() {
        ...
        let prompts = vec![];
        let result = exporter.export_directory(&prompts, &output_path, false).await;
        assert!(result.is_ok());
