...
use super::metrics::{PerformanceMetrics, Operation, MetricsSnapshot};
...
mod tests {
    ...
    async fn test_update_issue_records_metrics() {
        ...
        // Update the issue
        let updated_issue = storage.update_issue(issue.number.value(), "Updated content".to_string()).await.unwrap();
    ...
    async fn test_multiple_operations_metrics() {
        ...
        storage.get_issue(issue1.number.value()).await.unwrap();
        storage.get_issue(issue2.number.value()).await.unwrap();
        
        storage.update_issue(issue1.number.value(), "Updated content".to_string()).await.unwrap();
        
        storage.mark_complete(issue2.number.value()).await.unwrap();
    ...
    async fn test_performance_analysis() {
        ...
        for i in 1..=5 {
            storage.create_issue(format!("issue_{}", i), format!("Content {}", i)).await.unwrap();
        }
        
        // Perform multiple read operations (should be faster than creates)
        for i in 1..=5 {
            storage.get_issue(i).await.unwrap();
        }
        ...
        println!("Fastest operation: {:?} ({}μs)", fastest, 
                 match fastest {
        ...
        println!("Slowest operation: {:?} ({}μs)", slowest,
                 match slowest {
                     Some(Operation::Create) => snapshot.avg_create_time,
                     Some(Operation::Read) => snapshot.avg_read_time,
                     Some(Operation::Update) => snapshot.avg_update_time,
    ...
    async fn test_concurrent_operations_metrics() {
        ...
        for _ in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                storage_clone.get_issue(issue_number).await.unwrap();
            });
            handles.push(handle);
        }
