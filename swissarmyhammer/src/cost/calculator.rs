...
use crate::cost::{ApiCall, CostSession, CostError};
...
pub mod default_rates {
    ...
    /// Input token cost per token for Sonnet model (configurable)
    pub const SONNET_INPUT_TOKEN_COST: &str = "0.000015";
    /// Output token cost per token for Sonnet model (configurable) 
    pub const SONNET_OUTPUT_TOKEN_COST: &str = "0.000075";

    /// Input token cost per token for Opus model (configurable)
    pub const OPUS_INPUT_TOKEN_COST: &str = "0.000075";
    /// Output token cost per token for Opus model (configurable)
    pub const OPUS_OUTPUT_TOKEN_COST: &str = "0.000375";

    /// Input token cost per token for Haiku model (configurable)
    pub const HAIKU_INPUT_TOKEN_COST: &str = "0.0000025";
    /// Output token cost per token for Haiku model (configurable)
    pub const HAIKU_OUTPUT_TOKEN_COST: &str = "0.0000125";

    /// Get default Sonnet pricing rates
    pub fn sonnet_rates() -> (Decimal, Decimal) {
        (
            Decimal::from_str(SONNET_INPUT_TOKEN_COST).unwrap(),
            Decimal::from_str(SONNET_OUTPUT_TOKEN_COST).unwrap(),
        )
    }
    ...
    pub fn opus_rates() -> (Decimal, Decimal) {
        (
            Decimal::from_str(OPUS_INPUT_TOKEN_COST).unwrap(),
            Decimal::from_str(OPUS_OUTPUT_TOKEN_COST).unwrap(),
        )
    }
    ...
    pub fn haiku_rates() -> (Decimal, Decimal) {
        (
            Decimal::from_str(HAIKU_INPUT_TOKEN_COST).unwrap(),
            Decimal::from_str(HAIKU_OUTPUT_TOKEN_COST).unwrap(),
        )
    }
}
...
impl PaidPlanConfig {
    ...
    pub fn get_rates_for_model(&self, model: &str) -> &PricingRates {
        ...
        // Try fuzzy matching for model variations
        for (model_key, rates) in &self.model_rates {
            if model.contains(model_key) || model_key.contains(model) {
                return rates;
            }
        }
...
impl PricingRates {
    ...
    pub fn from_strings(
        ...
        let input_cost = Decimal::from_str(input_cost_str).map_err(|e| CostError::InvalidInput {
            message: format!("Invalid input token cost '{}': {}", input_cost_str, e),
        })?;

        let output_cost = Decimal::from_str(output_cost_str).map_err(|e| CostError::InvalidInput {
...
mod tests {
    ...
    fn test_pricing_rates_creation() {
        ...
        assert_eq!(rates.input_token_cost, Decimal::from_str("0.000015").unwrap());
        assert_eq!(rates.output_token_cost, Decimal::from_str("0.000075").unwrap());
    }
    ...
    fn test_paid_plan_config() {
        ...
        assert_eq!(sonnet_rates, config.model_rates.get("claude-3-sonnet").unwrap());
    ...
    fn test_max_plan_config() {
        ...
        let config_with_estimates = MaxPlanConfig::new_with_estimates(
            true,
            PaidPlanConfig::new_with_defaults(),
        );
    ...
    fn test_cost_calculation() {
        ...
        assert_eq!(calculation.output_cost, Decimal::from_str("0.0025").unwrap());
    ...
    fn test_edge_cases() {
        ...
        // Large token counts
        let result = calculator.calculate_tokens_cost(u32::MAX / 2, u32::MAX / 2, "claude-3-sonnet");
        assert!(result.is_ok());
        let calculation = result.unwrap();
        assert!(calculation.total_cost > Decimal::ZERO);
    ...
    fn test_decimal_precision() {
        ...
        assert_eq!(calculation.total_cost, calculation.input_cost + calculation.output_cost);
    }
