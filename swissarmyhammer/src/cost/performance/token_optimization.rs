...
use crate::cost::{CostError, TokenUsage, ConfidenceLevel};
...
impl TokenCache {
    ...
    fn evict_lru(
        &self, 
        cache: &mut HashMap<u64, TokenCacheEntry>, 
        stats: &mut TokenCacheStats
    ) {
        // Simple LRU: collect keys with their sort criteria first
        let mut entries: Vec<(u64, usize, Instant)> = cache.iter()
...
impl SimdTokenEstimator {
    ...
    pub fn estimate_tokens(&self, text: &str) -> u32 {
        if self.simd_enabled {
            self.estimate_tokens_simd(text)
        } else {
            self.estimate_tokens_scalar(text)
        }
    }

    /// SIMD-optimized token estimation (x86_64 only)
    #[cfg(target_arch = "x86_64")]
    fn estimate_tokens_simd(&self, text: &str) -> u32 {
        // Simplified SIMD implementation for demonstration
        // In production, this would use actual SIMD instructions for counting
        let bytes = text.as_bytes();
        let mut token_count = 0u32;
        
        // Process in 16-byte chunks (SIMD register size)
