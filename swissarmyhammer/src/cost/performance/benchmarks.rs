...
impl TokenCountingBenchmark {
    ...
    fn create_test_response(&self, size_multiplier: u64) -> String {
        ...
        format!("{}{}", base_response, "x".repeat((size_multiplier % 100) as usize))
    }
}
...
impl PerformanceBenchmark for TokenCountingBenchmark {
    ...
    fn run(&self, operations: u64) -> BenchmarkResult {
        ...
        for i in 0..operations {
            ...
            if let Ok(usage) = counter.count_from_response(
                &response,
                None,
                "claude-3-sonnet-20241022",
            ) {
        ...
        result.add_metric("avg_tokens_per_response".to_string(), total_tokens as f64 / operations as f64);
...
impl MemoryUsageBenchmark {
    ...
    fn estimate_memory_usage(&self, tracker: &CostTracker) -> usize {
        ...
        for (session_id, session) in sessions {
            ...
            total_size += session.api_calls.capacity() * std::mem::size_of::<crate::cost::ApiCall>();
        }
...
impl PerformanceBenchmark for MemoryUsageBenchmark {
    ...
    fn run(&self, operations: u64) -> BenchmarkResult {
        ...
        result.add_metric("memory_growth_bytes".to_string(), (final_memory - initial_memory) as f64);
        result.add_metric("memory_per_operation".to_string(), 
...
impl AggregationBenchmark {
    ...
    fn setup_test_data(&self, tracker: &mut CostTracker, sessions: u64) {
        ...
        for i in 0..sessions {
            ...
            for j in 0..API_CALLS_PER_SESSION {
                let mut api_call = ApiCall::new(
                    "https://api.anthropic.com/v1/messages",
                    if j % 3 == 0 { "claude-3-opus" } else { "claude-3-sonnet" },
                ).unwrap();
                api_call.complete(
                    100 + j * 10,
                    200 + j * 5,
            ...
            tracker.complete_session(&session_id, crate::cost::CostSessionStatus::Completed).unwrap();
        }
    }
    ...
    fn calculate_aggregated_metrics(&self, tracker: &CostTracker) -> HashMap<String, f64> {
        ...
        metrics.insert("avg_tokens_per_session".to_string(), 
                      if session_count > 0 { total_tokens as f64 / session_count as f64 } else { 0.0 });
    ...
    pub fn generate_report(&self, results: &[BenchmarkResult]) -> String {
        ...
        for result in results {
            ...
            report.push_str(&format!("  Target: {}\n", 
                                   if result.meets_target(self.target_ms) { "✓ PASS" } else { "✗ FAIL" }));
        ...
        report.push_str(&format!("Overall: {}\n", 
                               if self.all_meet_targets(results) { "✓ ALL PASS" } else { "✗ SOME FAIL" }));
