...
use crate::cost::{
    ...
    tracker::{
        ApiCall, ApiCallId, ApiCallStatus, CostError, CostSessionId,
        CostSessionStatus, CostTracker, IssueId, MAX_API_CALLS_PER_SESSION, MAX_COST_SESSIONS,
    },
};
...
async fn test_complete_cost_tracking_workflow() {
    ...
    // Phase 1: Start multiple concurrent sessions for different issues
    let issue_ids: Vec<_> = (1..=5)
    ...
    // Phase 2: Add realistic API calls to each session
    ...
    for (i, session_id) in session_ids.iter().enumerate() {
        for j in 0..3 {
            ...
            let call_id = tracker
                ...
                .expect("Should add API call");
            all_call_ids.push((*session_id, call_id));
        }
    }

    // Phase 3: Complete API calls with varying success rates and realistic token counts
    for (call_index, (session_id, call_id)) in all_call_ids.iter().enumerate() {
        ...
        tracker
            ...
            .expect("Should complete API call");
    }

    // Phase 4: Calculate costs for each session
    let mut total_workflow_cost = Decimal::ZERO;
    for session_id in &session_ids {
        let session = tracker.get_session(session_id).unwrap();
        let cost_calculation = calculator
            .calculate_session_cost(session)
            .expect("Should calculate session cost");
    ...
    // Phase 5: Complete sessions with different outcomes
    for (i, session_id) in session_ids.iter().enumerate() {
        let status = if i % 2 == 0 {
        ...
        tracker
            ...
            .expect("Should complete session");
    }
...
async fn test_configuration_integration() {
    ...
    let max_with_estimates = MaxPlanConfig::new_with_estimates(true, PaidPlanConfig::new_with_defaults());
...
async fn test_concurrent_sessions() {
    ...
    // Phase 1: Start concurrent sessions
    for i in 0..NUM_SESSIONS {
        let issue_id = create_test_issue_id(&format!("concurrent-{}", i)).unwrap();
        let session_id = tracker.start_session(issue_id).unwrap();
        session_ids.push(session_id);
    }
    ...
    // Phase 2: Add API calls concurrently to all sessions
    ...
    // Phase 3: Complete all API calls with realistic timing
    ...
    // Phase 4: Verify all sessions have correct state
    for session_id in &session_ids {
        let session = tracker.get_session(session_id).unwrap();
...
async fn test_memory_management() {
    ...
    for i in 0..MAX_API_CALLS_PER_SESSION {
        let api_call = create_test_api_call(&format!("limit-test/{}", i), "claude-3-haiku").unwrap();
...
async fn test_error_handling_integration() {
    ...
    assert!(matches!(invalid_issue_result, Err(CostError::InvalidInput { .. })));
    
    let invalid_issue_result = IssueId::new("a".repeat(300));
    assert!(matches!(invalid_issue_result, Err(CostError::InvalidInput { .. })));
    
    let invalid_call_result = ApiCall::new("", "claude-3-sonnet");
    assert!(matches!(invalid_call_result, Err(CostError::InvalidInput { .. })));
...
async fn test_performance_characteristics() {
    ...
    assert!(session_creation_time < Duration::from_secs(5), 
        "Session creation took too long: {:?}", session_creation_time);
    assert!(api_creation_time < Duration::from_secs(10), 
        "API call creation took too long: {:?}", api_creation_time);
    assert!(completion_time < Duration::from_secs(10), 
        "API call completion took too long: {:?}", completion_time);
    assert!(calculation_time < Duration::from_secs(5), 
        "Cost calculation took too long: {:?}", calculation_time);
    assert!(session_completion_time < Duration::from_secs(5), 
        "Session completion took too long: {:?}", session_completion_time);
    assert!(total_time < Duration::from_secs(30), 
        "Total performance test took too long: {:?}", total_time);
...
async fn test_edge_cases_and_boundaries() {
    ...
    tracker.add_api_call(&large_session_id, large_token_call).unwrap();
    ...
    let mut unknown_model_call = create_test_api_call("unknown-model", "unknown-model-2024").unwrap();
    complete_api_call_with_realistic_data(&mut unknown_model_call, 100, 200, true);
    tracker.add_api_call(&unknown_session_id, unknown_model_call).unwrap();
...
async fn test_cost_calculation_precision() {
    ...
    let small_calculation = calculator.calculate_tokens_cost(1, 1, "claude-3-sonnet").unwrap();
    ...
    let large_calculation = calculator.calculate_tokens_cost(100_000, 50_000, "claude-3-sonnet").unwrap();
    ...
    let sonnet_calc = calculator.calculate_tokens_cost(1000, 1000, "claude-3-sonnet").unwrap();
    let opus_calc = calculator.calculate_tokens_cost(1000, 1000, "claude-3-opus").unwrap();
    let haiku_calc = calculator.calculate_tokens_cost(1000, 1000, "claude-3-haiku").unwrap();
