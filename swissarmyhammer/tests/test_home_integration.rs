...
mod test_helpers;
use test_helpers::{create_test_home_guard, get_test_swissarmyhammer_dir};
...
fn test_home_directory_override_works() {
    ...
    
    {
        ...
        let another_test = swissarmyhammer_dir.join("prompts").join("another-test.md.liquid");
        assert!(another_test.exists());
        
        // Check that test workflow exists
        let test_workflow = swissarmyhammer_dir.join("workflows").join("test-workflow.yaml");
...
fn test_prompt_loading_with_test_home() {
    ...
    let prompts = loader.load_directory(&test_dir).expect("Failed to load prompts");
...
fn test_prompt_resolver_with_test_home() {
    ...
    resolver.load_user_prompts(&mut library).expect("Failed to load user prompts");
    
    let prompts = library.list().expect("Failed to list prompts");
    let user_prompt_names: Vec<String> = prompts.iter()
        .map(|p| p.name.clone())
        .collect();
    ...
    assert!(user_prompt_names.contains(&"test-prompt".to_string()), "Missing test-prompt");
    assert!(user_prompt_names.contains(&"another-test".to_string()), "Missing another-test");
}