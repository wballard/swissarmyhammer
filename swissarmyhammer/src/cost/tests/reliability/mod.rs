...
async fn test_memory_leak_detection() {
    ...
    let test_future = async {
        ...
        tracing::info!("Memory threshold: {}, doubled threshold: {}", config.memory_threshold, config.memory_threshold * 2);
    ...
    match &result {
        Ok(Ok(_)) => tracing::info!("Test scenario 'memory_leak_detection' completed successfully in {:?}", scenario_duration),
        Ok(Err(e)) => tracing::error!("Test scenario 'memory_leak_detection' failed after {:?}: {}", scenario_duration, e),
        Err(_) => tracing::error!("Test scenario 'memory_leak_detection' timed out after {:?}", scenario_duration),
    }

    assert!(result.is_ok(), "Memory leak detection should complete within timeout");
    let scenario_result = result.unwrap();
    assert!(scenario_result.is_ok(), "Memory leak detection should succeed: {:?}", scenario_result);

    let (operation_count, final_stats) = scenario_result.unwrap();
    assert!(operation_count > 0, "Should perform some operations");
    
    tracing::info!("Memory leak test completed: {} operations, final stats: {:?}", 
        operation_count, final_stats);
}
...
async fn test_concurrent_session_safety() {
    ...
    let result = timeout(Duration::from_secs(120), async {
        ...
        let worker_handles: Vec<_> = (0..num_workers).map(|worker_id| {
            ...
            tokio::spawn(async move {
                ...
                for op_id in 0..operations_per_worker {
                    let issue_id = IssueId::new(format!("concurrent-safety-{}-{}", worker_id, op_id))?;
                    ...
                    for call_id in 0..num_calls {
                        let mut api_call = ApiCall::new(
                            format!("https://api.anthropic.com/v1/worker-{}-op-{}-call-{}", 
        ...
        for handle in worker_handles {
            let worker_result = handle.await.expect("Worker task should not panic").expect("Worker should complete successfully");
        ...
        assert_eq!(final_tracker.session_count(), expected_sessions, 
            "Should have created all expected sessions");
        assert_eq!(final_tracker.completed_session_count(), expected_sessions, 
            "All sessions should be completed");
        assert!(total_cost >= Decimal::ZERO, "Total cost should be non-negative");

        Ok::<(usize, Decimal), CostError>((total_operations, total_cost))
    }).await;

    assert!(result.is_ok(), "Concurrent safety test should complete: {:?}", result);
    let (total_operations, total_cost) = result.unwrap().unwrap();
    
    assert!(total_operations > 0, "Should perform operations");
    tracing::info!("Concurrent safety test: {} operations, total cost: {}", 
...
async fn test_resource_cleanup_effectiveness() {
    ...
    let result: Result<(usize, usize, usize, Duration), Box<dyn std::error::Error + Send + Sync>> = async {
        let shared_tracker = harness.get_shared_tracker();
        ...
        assert_eq!(current_sessions, session_count, "Should track all created sessions");
        assert_eq!(active_sessions, session_count - half_count, "Should have correct active count");
        assert_eq!(completed_sessions, half_count, "Should have correct completed count");

        let cleanup_duration = cleanup_start.elapsed();
        assert!(cleanup_duration <= config.max_cleanup_time, 
            "Cleanup should complete within acceptable time: {:?}", cleanup_duration);

        Ok((current_sessions, active_sessions, completed_sessions, cleanup_duration))
    }.await;

    let duration = start.elapsed();
    match &result {
        Ok(_) => tracing::info!("Test scenario 'resource_cleanup' completed successfully in {:?}", duration),
        Err(e) => tracing::error!("Test scenario 'resource_cleanup' failed after {:?}: {}", duration, e),
    }

    assert!(result.is_ok(), "Resource cleanup test should succeed: {:?}", result);
}
...
async fn test_graceful_degradation_under_load() {
    ...
    let result: Result<(usize, usize, f64, Duration), Box<dyn std::error::Error + Send + Sync>> = async {
        let shared_tracker = harness.get_shared_tracker();
        ...
        assert!(success_rate >= 0.3, "Success rate should be at least 30% under load: {:.2}%", success_rate * 100.0);
        assert!(average_response_time <= Duration::from_secs(5), "Average response time should be reasonable: {:?}", average_response_time);

        tracing::info!("Load test results: {:.1}% success rate, avg response time: {:?}", 
            success_rate * 100.0, average_response_time);

        Ok((success_count, failure_count, success_rate, average_response_time))
    }.await;

    let duration = start.elapsed();
    match &result {
        Ok(_) => tracing::info!("Test scenario 'graceful_degradation' completed successfully in {:?}", duration),
        Err(e) => tracing::error!("Test scenario 'graceful_degradation' failed after {:?}: {}", duration, e),
    }

    assert!(result.is_ok(), "Graceful degradation test should succeed: {:?}", result);
}
...
async fn test_sustained_operation_stability() {
    ...
    let test_future = async {
        ...
        while start_time.elapsed() < config.test_duration {
            ...
            tracing::debug!("Stability check - Total: {}, Active: {}, Completed: {}", 
                current_sessions, active_sessions, total_sessions_completed);

            // Verify system stability
            assert_eq!(active_sessions, 0, "All sessions should be completed in each batch");
            assert!(current_sessions >= total_sessions_completed, "Session tracking should be consistent");
        }

        // Final stability validation
        let final_tracker = shared_tracker.lock().await;
        assert_eq!(final_tracker.completed_session_count(), total_sessions_completed, 
            "Final completed count should match");
        assert_eq!(final_tracker.active_session_count(), 0, 
            "No active sessions should remain");

        Ok::<_, Box<dyn std::error::Error + Send + Sync>>((total_sessions_created, total_sessions_completed))
    };
    ...
    let scenario_duration = scenario_start.elapsed();
    match &result {
        Ok(Ok(_)) => tracing::info!("Test scenario 'sustained_stability' completed successfully in {:?}", scenario_duration),
        Ok(Err(e)) => tracing::error!("Test scenario 'sustained_stability' failed after {:?}: {}", scenario_duration, e),
        Err(_) => tracing::error!("Test scenario 'sustained_stability' timed out after {:?}", scenario_duration),
    }

    assert!(result.is_ok(), "Sustained stability test should complete: {:?}", result);
    let scenario_result = result.unwrap();
    assert!(scenario_result.is_ok(), "Sustained stability should succeed: {:?}", scenario_result);

    let (created, completed) = scenario_result.unwrap();
    assert_eq!(created, completed, "All created sessions should be completed");
    assert!(created > 0, "Should create some sessions during test");
    
    tracing::info!("Sustained stability test: {} sessions created and completed", created);
}
