...
use crate::cost::{
    test_utils::storage_helpers::{
        ExpectedCostData, MultiBackendValidator, PerformanceValidator, StorageResults,
        StorageTestConfig, StorageTestHarness,
    },
...
pub mod test_constants {
    ...
    /// Number of sessions for performance testing
    pub const PERF_TEST_SESSION_COUNT: usize = 10;

    /// Number of API calls per session in performance tests
    pub const PERF_TEST_CALLS_PER_SESSION: usize = 5;

    /// Number of API calls for large dataset testing
    pub const LARGE_DATASET_CALL_COUNT: usize = 50;

    /// Delay between concurrent worker operations (milliseconds)
    pub const WORKER_DELAY_BASE_MS: u64 = 50;

    /// Minimum expected operation duration for performance tests (milliseconds)
    pub const MIN_OPERATION_DURATION_MS: u64 = 10;
}

/// Configurable performance thresholds for testing
#[derive(Debug, Clone)]
pub struct PerformanceThresholds {
    /// Maximum acceptable duration for bulk storage operations
    pub max_bulk_storage_duration: Duration,
    /// Maximum acceptable duration for single storage operation
    pub max_single_storage_duration: Duration,
    /// Maximum acceptable duration for concurrent operations
    pub max_concurrent_operation_duration: Duration,
}

impl Default for PerformanceThresholds {
    fn default() -> Self {
        Self {
            max_bulk_storage_duration: Duration::from_secs(30),
            max_single_storage_duration: Duration::from_secs(5),
            max_concurrent_operation_duration: Duration::from_secs(10),
        }
    }
}

impl PerformanceThresholds {
    /// Create performance thresholds optimized for CI/CD environments
    pub fn ci_optimized() -> Self {
        Self {
            max_bulk_storage_duration: Duration::from_secs(60), // More lenient for CI
            max_single_storage_duration: Duration::from_secs(10),
            max_concurrent_operation_duration: Duration::from_secs(20),
        }
    }

    /// Create strict performance thresholds for development
    pub fn strict() -> Self {
        Self {
            max_bulk_storage_duration: Duration::from_secs(15),
            max_single_storage_duration: Duration::from_secs(2),
            max_concurrent_operation_duration: Duration::from_secs(5),
        }
    }
}

/// Standardized database feature flag handling
macro_rules! with_database_feature {
    ($harness:expr, $test_config:expr, $code:block) => {
        #[cfg(feature = "database")]
        if $test_config.enable_database {
...
/// Helper to check if database storage should be expected
#[cfg(feature = "database")]
fn should_test_database(config: &StorageTestConfig) -> bool {
    config.enable_database
}

#[cfg(not(feature = "database"))]
fn should_test_database(_config: &StorageTestConfig) -> bool {
    false
}
...
async fn test_complete_cost_storage_workflow() {
    ...
    let issue_id = harness
        .create_test_issue("complete-workflow")
        .await
        .unwrap();
    let session_id = harness.start_cost_session(issue_id).unwrap();

    // Phase 2: Add and complete API calls
    let call_ids = harness
        .add_test_api_calls(&session_id, test_constants::BASIC_API_CALLS_PER_SESSION)
        .await
        .unwrap();
    ...
    assert!(markdown.contains(&format!(
        "**Total API Calls**: {}",
        test_constants::BASIC_API_CALLS_PER_SESSION
    )));

    // Verify database storage if enabled
    if should_test_database(&harness.test_config) {
...
async fn test_issue_markdown_cost_integration() {
    ...
    let issue_id = harness
        .create_test_issue("markdown-integration")
        .await
        .unwrap();
...
async fn test_metrics_cost_aggregation() {
    ...
    for i in 0..3 {
        let issue_id = harness
            .create_test_issue(&format!("metrics-test-{}", i))
            .await
            .unwrap();
        let session_id = harness.start_cost_session(issue_id).unwrap();
        ...
        let call_ids = harness
            .add_test_api_calls(&session_id, call_count)
            .await
            .unwrap();
        harness.complete_api_calls(&session_id, &call_ids).unwrap();
...
async fn test_optional_database_storage() {
    ...
    let issue_id = harness_with_db
        .create_test_issue("db-enabled")
        .await
        .unwrap();
    let session_id = harness_with_db.start_cost_session(issue_id).unwrap();

    let call_ids = harness_with_db
        .add_test_api_calls(&session_id, 3)
        .await
        .unwrap();
    harness_with_db
        .complete_api_calls(&session_id, &call_ids)
        .unwrap();
    ...
    let issue_id = harness_without_db
        .create_test_issue("db-disabled")
        .await
        .unwrap();
    let session_id = harness_without_db.start_cost_session(issue_id).unwrap();

    let call_ids = harness_without_db
        .add_test_api_calls(&session_id, 3)
        .await
        .unwrap();
    harness_without_db
...
async fn test_storage_backend_consistency() {
    ...
    let total_input = predictable_calls
        .iter()
        .map(|(input, _)| *input)
        .sum::<u32>();
    let total_output = predictable_calls
        .iter()
        .map(|(_, output)| *output)
        .sum::<u32>();
    ...
    if let Some(markdown) = &storage_results.markdown_content {
        assert!(validator
            .validate_markdown_content(&session_id.to_string(), markdown)
            .unwrap());
    }

    // Validate database data if feature enabled
    with_database_feature!(harness, harness.test_config, {
        if storage_results.database_stored {
            assert!(validator
                .validate_database_data(&session_id.to_string(), database)
                .await
                .unwrap());
        }
    });
...
async fn test_storage_error_recovery() {
    ...
    let issue_id = partial_harness
        .create_test_issue("partial-backend")
        .await
        .unwrap();
    let session_id = partial_harness.start_cost_session(issue_id).unwrap();

    let call_ids = partial_harness
        .add_test_api_calls(&session_id, 2)
        .await
        .unwrap();
    partial_harness
        .complete_api_calls(&session_id, &call_ids)
...
async fn test_configuration_flexibility() {
    ...
    let mut harness_all = StorageTestHarness::with_config(config_all.clone())
        .await
        .unwrap();
    ...
    let mut harness_markdown = StorageTestHarness::with_config(config_markdown_only.clone())
        .await
        .unwrap();
    let issue_id = harness_markdown
        .create_test_issue("config-markdown")
        .await
        .unwrap();
    let session_id = harness_markdown.start_cost_session(issue_id).unwrap();
    let call_ids = harness_markdown
        .add_test_api_calls(&session_id, 2)
        .await
        .unwrap();
    harness_markdown
...
/// Helper function for performance testing with configurable thresholds
async fn run_storage_performance_test(thresholds: PerformanceThresholds) {
    let mut harness = StorageTestHarness::with_config(StorageTestConfig {
        enable_database: true,
        enable_markdown_storage: true,
        enable_metrics_integration: true,
    ...
    const PERF_SESSION_COUNT: usize = test_constants::PERF_TEST_SESSION_COUNT;
    const CALLS_PER_SESSION: usize = test_constants::PERF_TEST_CALLS_PER_SESSION;
    ...
    for i in 0..PERF_SESSION_COUNT {
        let issue_id = harness
            .create_test_issue(&format!("perf-test-{}", i))
            .await
            .unwrap();
        let session_id = harness.start_cost_session(issue_id).unwrap();

        let call_ids = harness
            .add_test_api_calls(&session_id, CALLS_PER_SESSION)
            .await
            .unwrap();
        harness.complete_api_calls(&session_id, &call_ids).unwrap();
    ...
    let max_acceptable_duration = thresholds.max_bulk_storage_duration;
    assert!(performance_validator
        .assert_performance_bounds("bulk_storage_operations", max_acceptable_duration));
    ...
    let single_call_ids = harness
        .add_test_api_calls(&single_session_id, 3)
        .await
        .unwrap();
    harness
        .complete_api_calls(&single_session_id, &single_call_ids)
        .unwrap();
    ...
    let max_single_duration = thresholds.max_single_storage_duration;
    assert!(performance_validator
        .assert_performance_bounds("single_storage_operation", max_single_duration));
...
/// Test performance characteristics under storage load with default thresholds
#[tokio::test]
async fn test_storage_performance_validation() {
    run_storage_performance_test(PerformanceThresholds::default()).await;
}

/// Test storage performance with CI-optimized thresholds
#[tokio::test]
async fn test_storage_performance_validation_ci() {
    run_storage_performance_test(PerformanceThresholds::ci_optimized()).await;
}

/// Test storage performance with strict thresholds for development
#[tokio::test]
#[ignore] // Ignored by default as it's very strict - run explicitly during development
async fn test_storage_performance_validation_strict() {
    run_storage_performance_test(PerformanceThresholds::strict()).await;
}
    ...
    for worker_id in 0..3 {
        let handle = tokio::spawn(async move {
            ...
            sleep(Duration::from_millis(
                test_constants::WORKER_DELAY_BASE_MS * worker_id as u64,
            ))
            .await;
            ...
            (
                worker_id,
                storage_results.all_backends_success(&worker_harness.test_config),
            )
        });
...
async fn test_large_dataset_storage() {
    ...
    const LARGE_CALL_COUNT: usize = test_constants::LARGE_DATASET_CALL_COUNT;
    let call_ids = harness
        .add_test_api_calls(&session_id, LARGE_CALL_COUNT)
        .await
        .unwrap();
    harness.complete_api_calls(&session_id, &call_ids).unwrap();
...
mod test_validation {
    ...
    async fn test_storage_test_harness_functionality() {
        ...
        // Verify database initialization if feature enabled
        if should_test_database(&harness.test_config) {
            #[cfg(feature = "database")]
    ...
    fn test_multi_backend_validator() {
        ...
        assert!(validator
            .validate_markdown_content("test-session", valid_markdown)
            .unwrap());

        // Test invalid markdown content
        let invalid_markdown = "Cost Analysis\n**Total Cost**: $0.50\n**Total API Calls**: 2";
        assert!(!validator
            .validate_markdown_content("test-session", invalid_markdown)
            .unwrap());
    }
    ...
    fn test_performance_validator() {
        ...
        std::thread::sleep(Duration::from_millis(
            test_constants::MIN_OPERATION_DURATION_MS,
        ));
        let duration = validator.stop_timing("test_operation");

        assert!(duration.is_some());
        assert!(
            duration.unwrap() >= Duration::from_millis(test_constants::MIN_OPERATION_DURATION_MS)
        );
