...
use swissarmyhammer::issues::{
    Issue, IssueNumber, IssueStorage, FileSystemIssueStorage,
    CachedIssueStorage, InstrumentedIssueStorage, IssueCache,
    ...
    for cache_size in cache_sizes {
        for issue_count in &issue_counts {
            ...
            group.bench_with_input(
                BenchmarkId::new("cache_put", format!("cache{}_issues{}", cache_size, issue_count)),
            ...
            group.bench_with_input(
                BenchmarkId::new("cache_get_hit", format!("cache{}_issues{}", cache_size, issue_count)),
                &(*issue_count, cache_size),
                |b, _| {
                    b.iter(|| {
            ...
            group.bench_with_input(
                BenchmarkId::new("cache_get_miss", format!("cache{}_issues{}", cache_size, issue_count)),
                &(*issue_count, cache_size),
                |b, _| {
                    b.iter(|| {
...
fn benchmark_cache_lru_eviction(c: &mut Criterion) {
    ...
    let cache_sizes = vec![10, 50, 100];
    let issue_counts = vec![20, 100, 200]; // More issues than cache size to force eviction
    ...
    for (cache_size, issue_count) in cache_sizes.iter().zip(issue_counts.iter()) {
        ...
        group.bench_with_input(
            BenchmarkId::new("lru_eviction", format!("cache{}_issues{}", cache_size, issue_count)),
            ...
            |b, &count| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for i in 1..=count {
                            let _issue = cached_storage.create_issue(
                                format!("test_{}", i),
                                format!("Content {}", i)
                            ).await.unwrap();
                        }
            ...
            |b, &count| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for i in 1..=count {
                            let _issue = cached_storage.get_issue(i).await.unwrap();
                        }
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for i in 1..=batch_size {
                            let _issue = storage.create_issue(
                                format!("test_{}", i),
                                format!("Content {}", i)
                            ).await.unwrap();
                        }
            ...
            |b, &batch_size| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        // Individual gets
                        for i in 1..=batch_size {
                            let _issue = storage.get_issue(i).await.unwrap();
                        }
                    });
                });
            },
            ...
            |b, &batch_size| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        // Batch get
                        let numbers: Vec<u32> = (1..=batch_size).collect();
                        let _issues = storage.get_issues_batch(numbers).await.unwrap();
                    });
                });
            },
        );
            ...
            |b, &count| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        // Create issues
                        for i in 1..=count {
                            let _issue = instrumented_storage.create_issue(
                                format!("test_{}", i),
                                format!("Content {}", i)
                            ).await.unwrap();
                        }
                        ...
                        // Read issues
                        for i in 1..=count {
                            let _issue = instrumented_storage.get_issue(i).await.unwrap();
                        }
                        
                        // Update issues
                        for i in 1..=count {
                            let _issue = instrumented_storage.update_issue(
                                i,
            ...
            |b, &count| {
                b.iter(|| {
                    rt.block_on(async {
                        let (fs_storage, _temp) = setup_fs_storage();
                        let cached_storage = CachedIssueStorage::new(Box::new(fs_storage));
                        let instrumented_cached_storage = InstrumentedIssueStorage::new(Box::new(cached_storage));
                        
                        // Create issues
                        for i in 1..=count {
                            let _issue = instrumented_cached_storage.create_issue(
                                format!("test_{}", i),
                                format!("Content {}", i)
                            ).await.unwrap();
                        }
                        ...
                        for _ in 0..3 {
                            for i in 1..=count {
                                let _issue = instrumented_cached_storage.get_issue(i).await.unwrap();
                            }
                        }
