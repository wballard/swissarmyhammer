...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../workflows/builtin");
...
fn generate_builtin_workflows(out_dir: &str) {
    ...
    // Get the manifest directory (where Cargo.toml is located)
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    let builtin_dir = Path::new(&manifest_dir).join("../workflows/builtin");

    let mut code = String::new();
    code.push_str("// Auto-generated builtin workflows - do not edit manually\n");
    code.push_str("// Generated by build.rs from workflows/builtin directory\n\n");
