...
async fn debug_issue_completion_detection() {
    ...
    fs::write(&completed_issue_file, "# Direct Completed Issue\n\nThis is directly in complete dir").unwrap();
    
    // Create a regular active issue
    let _active_issue = issue_storage
        .write()
        .await
        .create_issue("active_issue".to_string(), "This is a regular active issue".to_string())
        .await
        .unwrap();
    ...
    println!("Completed count: {}, Active count: {}", completed_count, active_count);
