...
async fn test_nested_complete_directory_bug() {
    ...
    fs::write(&completed_issue_file, "# Completed Issue\n\nThis issue is completed").unwrap();
    ...
    let _active_issue = issue_storage
        .write()
        .await
        .create_issue("regular_active".to_string(), "This is a regular active issue".to_string())
        .await
        .unwrap();
    ...
    for issue in &all_issues {
        ...
        match issue_num {
            1 => {
                ...
                assert!(issue.completed, "Issue 000001 should be completed (in complete directory)");
                assert_eq!(issue.name, "legitimate_completed");
            },
            2 => {
                // The nested issue should NOT be completed despite having "complete" in its path
                assert!(!issue.completed, "Issue 000002 should NOT be completed (nested directory)");
                assert_eq!(issue.name, "nested_not_completed");
            },
            _ => {
                // The regular active issue should not be completed
                assert!(!issue.completed, "Regular active issue should not be completed");
