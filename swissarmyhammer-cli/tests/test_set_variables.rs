...
"#).unwrap();
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Variables: {\"context_var\": String(\"value1\")"));
}
...
"#).unwrap();
    ...
    cmd.assert()
        ...
        .stderr(predicate::str::contains("Hello John Doe! You are user number 12345"))
        .stderr(predicate::str::contains("Processing 25 items"))
        .stderr(predicate::str::contains("Workflow completed for John Doe (ID: 12345)"));
}
...
"#).unwrap();
    ...
    cmd.assert()
        .success()
        .stderr(predicate::str::contains("User: {{ username }}, Email: {{ email }}"));
}
...
"#).unwrap();
    ...
    cmd.assert()
        ...
        .stderr(predicate::str::contains("Invalid filter: {{ name | nonexistent_filter }}"))
