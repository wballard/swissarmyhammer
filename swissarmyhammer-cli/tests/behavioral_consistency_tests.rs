...
fn test_issue_create_output_consistency() -> Result<()> {
    ...
    assert!(
        ...
        "Should mention the issue name in output: {}",
        stdout
    );
    
    // Verify the issue was actually created (name-based system)
    let created_issue_path = temp_path
        .join("issues")
        .join("test_behavioral_issue.md");
    assert!(
...
fn test_memo_create_output_consistency() -> Result<()> {
    ...
    assert!(
        ...
        "Should reference the memo in output: {}",
        stdout
    );
    ...
    assert!(
        ...
        "Should contain memo ID in output: {}",
        stdout
    );
...
fn test_memo_list_output_consistency() -> Result<()> {
    ...
    assert!(
        ...
        "Should list the created memo: {}",
        stdout
    );
    
    // Should have some structure (table headers or organized format)
    assert!(
        stdout.contains("Title") || stdout.contains("ID") || stdout.contains("Created")
...
"#,
    ...
    assert!(
        ...
        "Should show indexing success message: {}",
        stdout
    );
    ...
    assert!(
        ...
        "Should contain numeric information about indexing: {}",
        stdout
    );
...
"#,
    ...
    if !stdout.trim().is_empty() {
        ...
        assert!(
            ...
            "Should contain search terms in results: {}",
            stdout
        );
...
fn test_error_message_consistency() -> Result<()> {
    ...
    assert!(
        stderr.contains("Error") || stderr.contains("error") || stderr.contains("invalid")
            || stderr.contains("not found"),
        "Should show appropriate error for invalid memo ID: {}",
        stderr
    );
...
fn test_help_output_consistency() -> Result<()> {
    ...
    assert!(stdout.contains("issue"), "Help should mention issue commands");
    assert!(stdout.contains("memo"), "Help should mention memo commands");
    assert!(stdout.contains("search"), "Help should mention search commands");
...
fn test_subcommand_help_consistency() -> Result<()> {
    ...
    assert!(
        ...
        "Issue help should contain major subcommands: {}",
        stdout
    );
    ...
    assert!(
        ...
        "Memo help should contain major subcommands: {}",
        stdout
    );
    ...
    assert!(
        ...
        "Search help should contain major subcommands: {}",
        stdout
    );
