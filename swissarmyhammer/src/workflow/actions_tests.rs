...
fn create_test_context() -> HashMap<String, Value> {
    ...
    context.insert("test_var".to_string(), Value::String("test_value".to_string()));
    ...
    context.insert("user_name".to_string(), Value::String("testuser".to_string()));
    context
}
...
fn create_context_with_special_chars() -> HashMap<String, Value> {
    let mut context = HashMap::new();
    context.insert("special_chars".to_string(), Value::String("hello\"world'test".to_string()));
...
mod log_action_tests {
    ...
    async fn test_log_action_execution_info() {
        ...
        let result = action.execute(&mut context).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Value::String("Test info message".to_string()));
        assert_eq!(context.get("last_action_result"), Some(&Value::Bool(true)));
    }
    ...
    async fn test_log_action_execution_warning() {
        ...
        let result = action.execute(&mut context).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Value::String("Test warning message".to_string()));
    }
    ...
    async fn test_log_action_execution_error() {
        ...
        let result = action.execute(&mut context).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Value::String("Test error message".to_string()));
    }
    ...
    async fn test_log_action_variable_substitution() {
        ...
        assert_eq!(message, Value::String("File: test.rs, User: testuser".to_string()));
    }
    ...
    async fn test_log_action_with_special_characters() {
        ...
        assert!(result.is_ok());
        let message = result.unwrap();
        assert_eq!(message, Value::String("Special: hello\"world'test".to_string()));
    }
}
...
mod set_variable_action_tests {
    ...
    fn test_set_variable_action_description() {
        let action = SetVariableAction::new("test_var".to_string(), "test_value".to_string());
        assert_eq!(action.description(), "Set variable 'test_var' to 'test_value'");
    }
    ...
    async fn test_set_variable_action_execution_string() {
        ...
        assert_eq!(context.get("new_var"), Some(&Value::String("new_value".to_string())));
        assert_eq!(context.get("last_action_result"), Some(&Value::Bool(true)));
    }

    #[tokio::test]
    async fn test_set_variable_action_execution_json() {
        let action = SetVariableAction::new("json_var".to_string(), r#"{"key": "value", "number": 42}"#.to_string());
        let mut context = HashMap::new();
    ...
    async fn test_set_variable_action_execution_invalid_json() {
        let action = SetVariableAction::new("invalid_json".to_string(), "invalid json {".to_string());
        let mut context = HashMap::new();
        ...
        assert_eq!(context.get("invalid_json"), Some(&Value::String("invalid json {".to_string())));
    }
    ...
    async fn test_set_variable_action_variable_substitution() {
        ...
        let result = action.execute(&mut context).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Value::String("Value: test_value".to_string()));
        assert_eq!(context.get("result"), Some(&Value::String("Value: test_value".to_string())));
    }

    #[tokio::test]
    async fn test_set_variable_action_json_substitution() {
        let action = SetVariableAction::new("json_result".to_string(), r#"{"file": "${current_file}", "number": ${number_var}}"#.to_string());
        let mut context = create_test_context();
...
mod sub_workflow_action_tests {
    ...
    fn test_sub_workflow_action_with_input() {
        ...
        assert_eq!(action.input_variables.get("input1"), Some(&"value1".to_string()));
        assert_eq!(action.input_variables.get("input2"), Some(&"value2".to_string()));
    }
...
mod error_handling_tests {
    ...
    fn test_action_error_display() {
        ...
        let error = ActionError::Timeout { timeout: Duration::from_secs(30) };
...
mod integration_tests {
    ...
    async fn test_action_execution_context_preservation() {
        ...
        let mut context = HashMap::new();
        context.insert("initial_value".to_string(), Value::String("initial".to_string()));
        ...
        assert_eq!(context.get("new_var"), Some(&Value::String("new_value".to_string())));
        assert_eq!(context.get("initial_value"), Some(&Value::String("initial".to_string())));
        assert_eq!(context.get("last_action_result"), Some(&Value::Bool(true)));
    ...
    async fn test_multiple_actions_sequence() {
        ...
        let actions: Vec<Box<dyn Action>> = vec![
            Box::new(SetVariableAction::new("step1".to_string(), "completed".to_string())),
            Box::new(LogAction::info("Step 1: ${step1}".to_string())),
            Box::new(SetVariableAction::new("step2".to_string(), "also_completed".to_string())),
        ...
        assert_eq!(context.get("step1"), Some(&Value::String("completed".to_string())));
        assert_eq!(context.get("step2"), Some(&Value::String("also_completed".to_string())));
    }
