...
mod mcp_integration_tests {
    ...
    async fn test_mcp_create_issue() {
        ...
        assert!(!issue_files.is_empty(), "Issue file should be created");

        // Verify the issue file contains the expected content
        let issue_file = &issue_files[0];
        let content = fs::read_to_string(issue_file).unwrap();
        assert!(content.contains("This is a test issue created via MCP"));
    }
}
