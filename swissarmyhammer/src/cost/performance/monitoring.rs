...
impl std::fmt::Debug for PerformanceMonitor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PerformanceMonitor")
            ...
            .field(
                "measurements_count",
                &self.measurements.read().unwrap().len(),
            )
...
impl PerformanceMonitor {
    ...
    fn update_statistics(&self) {
        ...
        global.total_duration_micros = measurements.iter().map(|m| m.duration_micros).sum();
    ...
    fn calculate_operation_stats(
        ...
        let successful_count = measurements.iter().filter(|m| m.success).count();
    ...
    fn calculate_percentiles(&self, measurements: &[&PerformanceMeasurement]) -> Percentiles {
        let mut durations: Vec<u64> = measurements.iter().map(|m| m.duration_micros).collect();
        durations.sort();
    ...
    fn calculate_performance_trend(
        ...
        let mut recent: Vec<f64> = measurements
            .iter()
            .rev()
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!(
            "Overall Avg Duration: {:.2}μs\n",
            global.overall_avg_duration_micros
        ));
        report.push_str(&format!(
            "Performance Trend: {} (confidence: {:.1}%)\n",
            if global.performance_trend.improving {
                "Improving"
            } else {
                "Degrading"
            },
...
impl PerformanceMetrics {
    ...
    pub fn record_token_counting(&self, duration: Duration, success: bool, token_count: u32) {
        self.monitor
            .record_token_counting(duration, success, token_count);
    }
...
mod tests {
    ...
    fn test_percentiles_calculation() {
        ...
        let stats = monitor
            .get_operation_stats(OperationType::TokenCounting)
            .unwrap();
